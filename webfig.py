...
class RC4:
    ...
    def set_key(self, cryptkey: list):
        ...
        K = [cryptkey[i % key_lenth] for i in range(0, 256)]
        for i in range(256):
    ...
    def encrypt(self, msg: bytes):
        i = 0
        new_msg = [self.gen() ^ msg[i] for i in range(len(msg))]
...
class WebFig(object):
    ...
    def handshake(self, username, password):
        ...
        if len(resp) == 24:
            ...
            self.handshake_v2()
            self.is_valid_credential()
        else:
            raise Exception("Unknown handshake type.")

    def is_valid_credential(self, need_response=False):
        # Perform the login
        try:
            if need_response:
            ...
            # Replace the following line with actual success criteria
    ...
    def handshake_v1(self, username, password):
        ...
        for j in range(0, 3 * 56, 56):
            key = [
                (pwdHash[i >> 3] << 8 | (pwdHash[(i >> 3) + 1])) >> 8 - (i & 7) & 0xFE
        ...
        r = requests.post(
            ...
            data=web_encode(
                self.tracker.id.to_bytes(4, "big") + (0).to_bytes(4, "big") + out
            ),
        )
        resp = web_decode(r.content)
    ...
    def handshake_v2(self):
        ...
        shared = self.gen_shared_secret(
            bytes(self.private)[::-1], self.server[1][::-1]
        )[::-1]
    ...
    def gen_shared_secret(self, private_key: bytes, server_public: bytes):
        return donna25519.PrivateKey(private_key).do_exchange(
            donna25519.PublicKey(server_public)
        )
    ...
    def _send(
        ...
        if not ignore_response and r is not None:
            return self.check_response(r, encoded)
        else:
            return None

    def check_response(self, r, encoded):
